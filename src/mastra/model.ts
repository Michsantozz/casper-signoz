...
// Fireworks as a real ai-sdk model via the OpenAI-compatible endpoint (same
// wiring as createChatModel). We use THIS — not Mastra's router magic-string —
// for the agent so we own a wrappable LanguageModel and can attach the LLM
// telemetry middleware (Mastra's own exporter never lands token usage in SigNoz;
// see llm-telemetry.ts). The OpenAI-compat path is proven to auth correctly
// against Fireworks (createChatModel/generateObject already relies on it).
...
// Returns a real ai-sdk model via the OpenAI provider pointed at Fireworks'
...
// Wrapped with the LLM telemetry middleware so these one-shot calls also emit a
// gen_ai.* span with real token usage to SigNoz.
export function createChatModel() {
  const provider = (process.env.MODEL_PROVIDER ?? "fireworks").toLowerCase();
  if (provider === "bedrock") return withLlmTelemetry(createBedrockModel());
  const fireworks = createOpenAI({
    baseURL: "https://api.fireworks.ai/inference/v1",
    apiKey: requireEnv("FIREWORKS_API_KEY"),
  });
  return withLlmTelemetry(
    fireworks.chat(process.env.FIREWORKS_MODEL_ID ?? DEFAULT_FIREWORKS_MODEL),
  );
}
